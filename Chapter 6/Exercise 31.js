...
function creditcard(){
    let user = prompt('Enter a credit card number as a long integer(13-16 digits)');
    let odd = 0;
    let even = 0;
    let two = 0;
    
    if (/^\d{13,16}$/.test(user)) {
        for (let i = user.length-2; i >= 0; i -= 2) {
            two = 2 * user[i];
            if (two > 9) {
                two = Math.floor(two / 10) + two % 10;
            }
            odd += two;
        }
        for (let i = user.length-1; i > 0; i -= 2) {
            even += Number(user[i]);
        }
        ((odd + even)% 10 == 0) ? console.log(user +' is valid'): console.log(user +' is invalid');
    }else{
        alert('The credit card number must have 13-16 digits:')
    }
}
creditcard();