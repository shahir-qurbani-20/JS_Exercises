...
function sqrt(){
    ...
    let diffrence = 1;
    let nextGuess = 0;
    let n = Number(prompt('Enter a number to approximate the square root:'))

    for (let i = 1; i > 0.0001; i=diffrence) {
        nextGuess = (lastGuess + n / lastGuess) / 2;
        diffrence = Math.abs(nextGuess-lastGuess);
        lastGuess = nextGuess;
    }
    console.log('Square root of '+n+' is approximatly '+lastGuess);
}
sqrt();