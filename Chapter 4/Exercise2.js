...
let x1 =Number(prompt('Enter x of point 1:')) * (Math.PI/180)
let y1 =Number(prompt('Enter y of point 1:')) * (Math.PI/180)
let x2 =Number(prompt('Enter x of point 2:')) * (Math.PI/180)
let y2 =Number(prompt('Enter y of point 2:')) * (Math.PI/180)


let distance = 6371.01 * Math.acos(
    Math.sin(x1) * Math.sin(x2) + Math.cos(x1) * Math.cos(x2) * Math.cos(y1 - y2)
);

alert('The distance between two points is '+ distance +' km')