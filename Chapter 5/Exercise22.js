...
let loan = Number(prompt('Enter the loan number:'));
let N = Number(prompt('Enter the number of years:'));
let rate = Number(prompt('Enter the Annual Interest Rate:'));
let MIR = rate / 1200
let interest = null
let principal = null
let Balance = loan
...
let MP = ((loan) * (rate / 1200) / (1 - (1 / (((rate / 1200) + 1) ** (12 * N)))));
let TP = MP * N * 12;

for(let p = 1 ; p <= N * 12 ; p++){

    interest = MIR * Balance

    principal = MP - interest

    Balance = Balance - principal

    chart+= String(p).padEnd(13)+String(interest.toFixed(2)).padEnd(13)+String(principal.toFixed(2)).padEnd(15)+Balance.toFixed(2)+'\n';
}

console.log('Monthly Payment: '+MP.toFixed(2)+'\nTotal Payment: '+TP.toFixed(2)+'\n\nPayment#     Interest     Principal      Balance\n'+chart);
