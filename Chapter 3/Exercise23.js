...
let x = Number(prompt('Enter a point with two coordinates.\nEnter x'))
let y = Number(prompt('Enter a point with two coordinates.\nEnter y'))

if(x <= 5 && x >= -5 && y <= 2.5 && y >= -2.5){
    alert(`Point (${x}.0 , ${y}.0) is in the rectangle.`)
}else(
    alert(`Point (${x}.0 , ${y}.0) is not in the rectangle.`)
)